  ...
  return (
    <>
      <header className="sticky top-0 z-20 flex h-16 items-center gap-4 border-b border-slate-200 bg-white/80 px-6 backdrop-blur">
        ...
        <button className="relative rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label="Notifications (coming soon)" title="Notifications — coming soon">
          <Bell className="h-5 w-5" />
          <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-slate-300" />
        </button>

        <button className="rounded-lg p-2 text-slate-500 hover:bg-slate-100" aria-label="Help (coming soon)" title="Help — coming soon">
        ...
        <div className="relative">
          <button
            ...
            className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full ring-2 ring-slate-200 transition-shadow hover:ring-slate-300"
            aria-label="Open profile menu"
          >
            {user?.avatar_url ? (
              <img src={user.avatar_url} alt={name} className="h-9 w-9 object-cover" referrerPolicy="no-referrer" />
            ) : (
              <span className="flex h-9 w-9 items-center justify-center bg-gradient-to-br from-brand-500 to-purple-600 text-sm font-semibold text-white">
                {initials}
              </span>
            )}
          </button>
          ...
          {profileOpen && (
            <>
              ...
              <div className="absolute right-0 top-11 z-40 w-64 rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
                <div className="flex items-center gap-3 rounded-lg px-3 py-2.5">
                  {user?.avatar_url ? (
                    <img src={user.avatar_url} alt={name} className="h-10 w-10 rounded-full object-cover" referrerPolicy="no-referrer" />
                  ) : (
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-brand-500 to-purple-600 text-sm font-semibold text-white">
                      {initials}
                    </div>
                  )}
